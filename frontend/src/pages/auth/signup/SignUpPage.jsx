...
const SignUpPage = () => {
  ...
  const { mutate, isError, isLoading, error: mutationError } = useMutation({
    mutationFn: async ({ email, username, fullName, password, accountType, additionalInfo, location }) => {
      const res = await fetch("/api/auth/signup", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ email, username, fullName, password, accountType, additionalInfo, location }),
      });
  ...
  const handleSubmit = (e) => {
    ...
    getLocation((location) => {
      const dataWithLocation = { ...formData, location };
