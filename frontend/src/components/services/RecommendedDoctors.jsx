...
const customStyles = {
  content: {
    ...
    width: '40%', // adjust this value to change the size of the modal
    height: '40%', // adjust this value to change the size of the modal
  },
};
function RecommendedDoctors() {
  ...
  return (
    <div>
      <h4 className="font-semibold text-center">Our Top Doctors</h4>
      <div className="flex overflow-x-scroll">
        {doctors.map((doctor) => (
          <div key={doctor.id} className="flex-shrink-0 w-56 md:w-72 mr-4">
          
            {/* doctor card content */}
            <div className="bg-gray-700 shadow rounded-lg p-2 relative">
              <button
                onClick={() => handleMenuClick(doctor.id)}
                className="absolute right-0 top-0 mt-2 mr-2"
              >
                <IoMdMore className="text-slate-950 h-6 w-6" />
              </button>
              {showMenu === doctor.id && (
            <div className="absolute right-0 mt-8 mr-2 bg-white text-black p-2 rounded shadow-lg">
            <p onClick={() => handleBookClick(doctor)}>Book</p>
            <p onClick={() => handleRateClick(doctor)}>Rate</p>
            <p onClick={() => handleViewClick(doctor)}>View</p>
            </div>
          )}
          <Modal
  isOpen={modalIsOpen}
      ...
      >
        ...
        <StarRating rating={rating} onRate={handleRate} />
              ...
              <img
                src={doctor.imageUrl}
                alt={doctor.name}
                className="w-full image-full h-40 object-cover rounded-lg"
              />
              <div className="mt-2">
                <h5 className="font-semibold text-sm text-slate-50">{doctor.name}</h5>
                <p className="text-xs text-white">{doctor.specialty}</p>
                <p className="text-xs text-white">{doctor.location}</p>
                <div className="flex items-center mt-2">
                  <span className="text-xs text-orange-500">Rating: {doctor.rating}</span>
